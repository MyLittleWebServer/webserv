...
{	
	while (1)
	{
		ssize_t n = recv(this->_sd, this->_buf, TRANSFER_LEN, 0);
		// std::cout << "n: " << n << '\n'; zoosuck
		if (n <= 0)
		{
			if (n < 0)
		...
		this->_buf[n] = '\0';
		this->_request += this->_buf;
		std::memset(this->_buf, 0, TRANSFER_LEN + 1);
		if (n < TRANSFER_LEN || recv(this->_sd, this->_buf, TRANSFER_LEN, MSG_PEEK) == -1)
		{
...
void	Client::sendData(std::map<int, Client> &clients)
{
	...
	if (it != clients.end() && this->_method->getResponseFlag() == true)
	{
		...
		while (1)
		{
			ssize_t n = send(this->_sd, \
							response.c_str() + (pos - begin), \
							TRANSFER_LEN, 0);
